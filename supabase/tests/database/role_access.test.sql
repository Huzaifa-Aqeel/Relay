...
select is((select free_role_id from public.organizations where id = pg_temp.id('org')),pg_temp.id('president'),'First Organization Role becomes the stable Free Role');
...
select is((select free_role_id from public.organizations where id = pg_temp.id('org')),pg_temp.id('president'),'Pro Role creation does not replace the stable Free Role');
...
select lives_ok($$select public.create_role_assignment_invite(pg_temp.id('treasurer'),'2028–2029')$$,'Owner can manage succession for any Role without becoming its holder');
...
select is(public.accept_role_assignment_invite(pg_temp.token('replacement')),pg_temp.id('treasurer_handoff'),'Role Holder can invite their own mid-year replacement into the same handoff');
...
select isnt(pg_temp.id('next_handoff'),pg_temp.id('treasurer_handoff'),'Role Holder can invite the next-period successor into a separate workspace');
select is((select count(*) from public.role_assignments where role_id = pg_temp.id('treasurer') and service_period = '2026–2027' and status = 'active'),0::bigint,'New-period acceptance atomically ends the outgoing assignment');
select is((select status from public.role_assignments where role_id = pg_temp.id('treasurer') and service_period = '2027–2028'),'active','New-period acceptance atomically activates the successor');
select is((select count(*) from public.role_assignments where role_id = pg_temp.id('treasurer') and status = 'active'),1::bigint,'Only one current holder remains across Role service periods');
...
select pg_temp.actor(3);
select ok(public.is_organization_member(pg_temp.id('org')),'Outgoing holder account and Organization membership remain intact');
select ok(not public.is_role_holder_for_handoff(pg_temp.id('treasurer_handoff')),'Outgoing holder loses old-workspace edit authority after new-period acceptance');
select is((select count(*) from public.sources),0::bigint,'Outgoing holder loses old-workspace private Source access');
select throws_ok($$select public.reopen_handoff_for_revision(pg_temp.id('treasurer_handoff'))$$,'42501','Active Role Assignment required','Outgoing holder cannot reopen the previous workspace');
set local role postgres;
select is((select created_by from public.knowledge_items where id = 'c0000000-0000-4000-8000-000000000001'),'a0000000-0000-4000-8000-000000000002'::uuid,'Succession preserves historical contributor attribution');
set local role authenticated;
select pg_temp.actor(3);
select throws_ok($$select public.create_role_assignment_invite(pg_temp.id('treasurer'),'2028–2029')$$,'42501','Only the Organization Owner or current Role Holder can invite for this Role','Previous-period holder cannot manage succession after the next holder accepts');
...
select pg_temp.actor(2);
select set_config('test.locked_invite',public.create_role_assignment_invite(pg_temp.id('treasurer'),'2028–2029')->>'token',true);
...
select is((select free_role_id from public.organizations where id = pg_temp.id('org')),pg_temp.id('president'),'Downgrade retains the designated Free Role');
select is((select count(*) from public.role_assignments where status = 'active'),2::bigint,'Downgrade preserves current assignments');
select ok(public.get_shared_handoff(pg_temp.token('public_token')) is not null,'Downgrade preserves recipient access');
select ok(not public.is_role_holder_for_handoff(pg_temp.id('next_handoff')),'Downgrade locks an assigned extra Role workspace');
select is((select count(*) from public.sources where handoff_id = pg_temp.id('treasurer_handoff')),0::bigint,'Locked Role Holder cannot read private Sources');
select is(public.preview_role_assignment_invite(pg_temp.token('locked_invite'))->>'planAvailable','false','Pending extra-Role invite reports the expired plan before acceptance');
select throws_ok($$select public.accept_role_assignment_invite(pg_temp.token('locked_invite'))$$,'P0001','RELAY_PRO_REQUIRED:role_access','Invite acceptance rechecks the Role plan entitlement');
select throws_ok($$select public.create_role_assignment_invite(pg_temp.id('treasurer'),'2030–2031')$$,'P0001','RELAY_PRO_REQUIRED:role_access','Free Organization cannot issue invites for a locked extra Role');
...
select pg_temp.actor(1);
select ok(public.is_role_holder_for_handoff(pg_temp.id('president_handoff')),'Designated Free Role remains usable by its assigned holder');
...
select pg_temp.actor(2);
set local role authenticated;
select ok(public.is_role_holder_for_handoff(pg_temp.id('next_handoff')),'Restoring Pro unlocks the preserved extra Role assignment');
set local role postgres;
select * from finish();
rollback;
